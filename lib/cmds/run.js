...
class RunCommand extends Command {
  ...
  static async runObject(scripts, options) {
    const legacyReporting = options.legacyReporting;
    ...
    if (options.dotenv) {
      ...
      try {
        ...
        console.log(`WARNING: could not read dotenv file: ${options.dotenv}`);
      }
      dotenv.config({ path: dotEnvPath });
    }

    try {
      const inputFiles = Array.isArray(scripts) ? scripts : [scripts];
      const script = await prepareTestExecutionPlan(inputFiles, options);
      ...
      let platformConfig = {};
      if (options['platform-opt']) {
        for (const opt of options['platform-opt']) {
          const [k, v] = opt.split('=');
          platformConfig[k] = v;
        }
      }
      ...
      const launcherOpts = {
        platform,
        platformConfig,
        mode: platform === 'local' ? 'distribute' : 'multiply',
      ...
      if (process.env.CUSTOM_REPORTERS) {
        ...
        customReporterNames.forEach(function (name) {
          ...
          const reporter = createReporter(launcher.events, options);
      ...
      launcher.events.on('stats', function (stats) {
        if (artillery.runtimeOptions.legacyReporting || legacyReporting) {
      ...
      launcher.events.on('done', async function (stats) {
        ...
        if (options.output) {
          ...
          if (_.isFunction(options.output)) {
            ...
            let logfile = getLogFilename(options.output);
            if (!options.quiet) {
      ...
      await sendTelemetry(script, options);
      launcher.run();
...
async function prepareTestExecutionPlan(inputFiles, flags) {
  ...
  for (const fn of inputFiles) {
    if (typeof fn === 'string' || fn instanceof String) {
      ...
      script1 = await checkConfig(_.merge(script1, fn), 'script.yml', flags);
    }
  }
